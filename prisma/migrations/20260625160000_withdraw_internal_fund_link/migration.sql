...
CREATE TABLE "new_internal_funds" (
    ...
    CONSTRAINT "internal_funds_withdrawTransactionId_fkey" FOREIGN KEY ("withdrawTransactionId") REFERENCES "withdraw_transactions" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO "new_internal_funds" ("amount", "assetId", "blockNo", "completedAt", "confirmations", "confirmedAt", "createdAt", "effectiveGasPrice", "feeAmount", "fromAddress", "fromIban", "fromWalletId", "gasUsed", "id", "internalFundNo", "internalTransactionId", "legSeq", "netAmount", "nonce", "providerTxnId", "referenceNo", "sentAt", "status", "statusHistory", "swapTransactionId", "toAddress", "toIban", "toWalletId", "txHash", "updatedAt") SELECT "amount", "assetId", "blockNo", "completedAt", "confirmations", "confirmedAt", "createdAt", "effectiveGasPrice", "feeAmount", "fromAddress", "fromIban", "fromWalletId", "gasUsed", "id", "internalFundNo", "internalTransactionId", "legSeq", "netAmount", "nonce", "providerTxnId", "referenceNo", "sentAt", "status", "statusHistory", "swapTransactionId", "toAddress", "toIban", "toWalletId", "txHash", "updatedAt" FROM "internal_funds";
...
CREATE INDEX "internal_funds_withdrawTransactionId_idx" ON "internal_funds"("withdrawTransactionId");
